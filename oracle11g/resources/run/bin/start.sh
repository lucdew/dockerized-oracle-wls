...
export ORACLE_HOME=/opt/oracle/product/11.2.0.1/dbhome_1
export PATH=$PATH:$ORACLE_HOME/bin

echo "SPFILE='/mnt/database/dbs/spfile${ORACLE_SID}.ora'" > /opt/oracle/product/11.2.0.1/dbhome_1/dbs/init${ORACLE_SID}.ora
...
function initdb {
  ...
  if [ -d /mnt/database/oradata ]; then
    ...
    # Create the database
    /opt/oracle/product/11.2.0.1/dbhome_1/bin/dbca -silent -gdbname ${ORACLE_SID} -sid ${ORACLE_SID} -responseFile /home/oracle/dbca.rsp
...
case "$COMMAND" in
  ...
  *)
    ...
    echo "  docker run -e COMMAND=initdb -e ORACLE_SID=FOO -v /tmp/db-FOO:/mnt/database oracle11g"
    echo ""
    echo "  To start sqlplus as sys in the database, and shut it down afterwards:"
    echo "  docker run -i -t -e COMMAND=sqlpluslocal -e ORACLE_SID=FOO -v /tmp/db-FOO:/mnt/database oracle11g"
    echo ""
    echo "  To run all *.sql scripts in /tmp/sql in the database, and shut it down afterwards:"
    echo "  docker run -e COMMAND=runsqllocal -e ORACLE_SID=FOO -v /tmp/db-FOO:/mnt/database -v /tmp/sql:/mnt/sql oracle11g"
    echo ""
    echo "  To start the database:"
    echo "  docker run -d -e COMMAND=rundb -e ORACLE_SID=FOO -v /tmp/db-FOO:/mnt/database -P --name db1 oracle11g"
    echo ""
    echo "  To connect to the database FOO running in container db1 with sqlplus:"
    echo "  docker run -i -t -e COMMAND=sqlplusremote -e ORACLE_SID=FOO -e ORACLE_USER=system -e ORACLE_PASSWORD=password --link db1:remotedb -P oracle11g"
    echo ""
    echo "  To run all *.sql scripts in /tmp/sql in the database FOO running in container db1:"
    echo "  docker run -e COMMAND=runsqlremote -e ORACLE_SID=FOO -e ORACLE_USER=system -e ORACLE_PASSWORD=password --link db1:remotedb -v /tmp/sql:/mnt/sql oracle11g"
    exit 1
    ;;
esac
